...
**Status snapshot**: v0.1.4 shipped 2026-07-17 with a Lockdown Mode detection fix (users who enabled LM were still seeing FAIL because the check read preference domains that don't exist). macOS DMG is freshly notarized. Windows .exe is CI-built but still **unsigned** (SmartScreen warning until an EU code-signing cert is procured). Apple Developer legal agreements refreshed on 2026-07-17 — notarytool works again.
...
## Uncommitted working-tree changes (LEAVE ALONE unless Luis asks)

```
 M .github/workflows/release-windows.yml   ← Luis's own edit
 M README.md                                ← Luis's own edit
 M ingest/cc_ingest.py                      ← Luis's ongoing ingest work
?? SECURITY.md                              ← new, not yet added
```

`HANDOFF.md` itself is also uncommitted — Luis may or may not want it in the repo. Ask before staging it.

Do not stage/commit any of these on Luis's behalf. He's iterating on ingest and adding a SECURITY.md. If you touch anything overlapping, sync with him first.

**Also uncommitted outside this repo**: same Lockdown Mode fix has been applied to `../securityscan-usb/engine/checks_macos/cat02_malware.py`. That folder is not a git repo — Luis distributes SSU by rsync to a physical USB. He needs to run the manual sync to propagate the fix.
...
| 4 | ingest/ pipeline in-progress (Luis's work) | Don't rearchitect. If asked to help, read `PLAN.md` first for the intended C-LAB inbox flow. |
...
Rewritten 2026-07-17 after shipping v0.1.4 (Lockdown Mode detection fix; Apple agreements refreshed). Previous update: 2026-06-11 after v0.1.3 (UTF-8 encoding fix). Update this file when the state drifts materially — especially: version number, open issues, recently fixed bugs, and any change to the release flow.
